 ...
 * longShortYards: positive = flew past the target, negative = came up short.
 * Both are null when there's no target for this shot.
 */
export function computeDispersion(start, target, end) {
  const carryYards = distanceYards(start, end);
  ...
  const lateralYards = carryYards * Math.sin(angleDiffRad);
  const longComponent = carryYards * Math.cos(angleDiffRad);
  const longShortYards = longComponent - targetDistanceYards;

  return { carryYards, targetDistanceYards, lateralYards, longShortYards };
}
