...
create table shots (
  ...
  -- GPS/tap distance from shot start to the point you marked. What that
  -- point represents depends on dispersion_basis below — you choose whether
  -- you're marking where the ball finished (total) or where it landed
  -- (carry), since at a place like Topgolf you can't walk out to check.
  total_yards numeric,
  carry_yards numeric,
  -- Which of the two columns above was actually measured by marking a
  -- point on the map/GPS (as opposed to typed in by hand afterward). This
  -- is what lateral_yards/long_short_yards are computed from.
  dispersion_basis text not null default 'total' check (dispersion_basis in ('total', 'carry')),
  target_distance_yards numeric,
  lateral_yards numeric,
  long_short_yards numeric,
  created_at timestamptz not null default now(),
  constraint shots_total_or_carry_present check (total_yards is not null or carry_yards is not null)
);
